...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // dogsJuliaCopy=dogsJulia.slice();
  console.log(dogsJuliaCopy.splice(-2, 3));
...
const calcAverageHumanAge = function (ages) {
  ...
  const averageAge =
    dogToHumanAge.reduce((acc, curr) => acc + curr) / dogToHumanAge.length;
...
const calcAverageHumanAgeArrow = function (ages) {
  const averageAge = ages
    .map(ele => (ele <= 2 ? ele * 2 : ele * 4 + 16))
    .filter(ele => ele >= 18)
    .reduce((acc, curr, val, arr) => acc + curr / arr.length, 0);
  // const averageAge = dogToHumanAge.reduce(
  //   (acc, curr, i, array) => acc + curr / array.length,0
  // );
  // console.log(averageAge);
  return averageAge;
};
